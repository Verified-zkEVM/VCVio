...
`lintegral_evalDist_length_drawUntil` identifies its expected number of draws with the negative
hypergeometric recursion `NegHypergeom.expectedDraws`, and `lintegral_evalDist_length_drawUntil_le`
...
The draw count is observed by mapping `List.length` over the loop, then integrating against its
output measure. The pool's value type needs no measurable-space instance.
-/
...
theorem lintegral_evalDist_length_cons_map (y : S) (mc : ProbComp (List S)) :
    (∫⁻ n, (n : ENNReal) ∂𝒟[List.length <$> ((y :: ·) <$> mc)]) =
      (∫⁻ n, (n : ENNReal) ∂𝒟[List.length <$> mc]) + 1 := by
  ...
  | succ n ih =>
      ...
      | succ r =>
        ...
        have hstep : ∀ i : Fin (xs.length + 1),
            (∫⁻ n, (n : ENNReal) ∂𝒟[List.length <$>
              ((((x :: xs)[(i : ℕ)]'(by simpa using i.isLt)) :: ·) <$>
                drawUntil accept
          ...
          rw [lintegral_evalDist_length_cons_map, ih _ _ hlen]
          congr 1
        ...
        rw [lintegral_evalDist_uniformFin]
        ...
        rw [show (∑ i : Fin (xs.length + 1),
          ...
          show xs.length + 1 - G = B from by omega, ← hG, ← hB]
...
theorem lintegral_evalDist_length_drawUntil_le (accept : S → Bool) (r : ℕ) (l : List S) :
    (∫⁻ n, (n : ENNReal) ∂𝒟[List.length <$> drawUntil accept r l])
      ≤ r * (l.length + 1) / (l.countP accept + 1) := by
  rw [lintegral_evalDist_length_drawUntil accept l.length r l rfl]
