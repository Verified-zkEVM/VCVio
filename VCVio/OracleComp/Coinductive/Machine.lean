...
abbrev OracleMachine (spec : OracleSpec.{u, u} ι) (α β : Type u) :=
  PFunctor.PointedMachine spec.toPFunctor α β
...
theorem runK_of_output_eq_some (H : QueryImpl spec m) {s : M.State} {b : β}
  ...
  PointedMachine.runWith_output_some M H k hb
...
/-- A machine implements a program at fuel `k` when its fuelled run agrees with
`simulateQ` of the program against every query implementation, in every lawful monad.
The `m := SPMF` instance (agreement against every `ProbHandler`) already pins down the
program's full distribution semantics — randomized oracles sample each query
independently, while a deterministic handler answers repeated queries identically — but
the monad-parametric form is the one stateful game transfer consumes
(`m := StateT σ SPMF`), and it yields the deterministic form as the `m := Id` instance. -/
...
theorem IsSimulation.runK_eq {M : OracleMachine spec α β}
  ...
  | pure b =>
    ...
    show (pure (some b) : m (Option β)) = some <$> simulateQ H (pure b)
