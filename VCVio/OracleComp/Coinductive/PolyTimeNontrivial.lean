...
theorem exists_poly_realizable_of_implements
  ...
  set g : M.State → M.State := M.stepD hdl with hg
  ...
  have hg_upd : ∀ s, g s = M.updateFlat (s, ⟨M.expose s, true⟩) := by
    ...
    | inl b =>
      simp only [OracleMachine.stepD, OracleMachine.updateFlat, hview, Sum.elim_inl]
    | inr q =>
      obtain ⟨t, cont⟩ := q
      simp only [OracleMachine.stepD, OracleMachine.updateFlat, hview, Sum.elim_inr,
        M.expose_of_view_query hview, hhdl, OracleHandler.ofFn_apply]
      rfl
  ...
  have hcorrect : ∀ x, M.output ((g^[k] ∘ M.init) x) = some (f n x) := by
    intro x
    have key := (himpl n).runWithInput_eq (m := Id) hdl.toQueryImpl x
    rw [Function.comp_apply, hg,
      ← M.runWith_eq_output_iterate_stepD hdl k (M.init x)]
    rw [show M.runWith hdl.toQueryImpl k (M.init x)
        = PFunctor.DynSystem.DynComputation.runWithInput M hdl.toQueryImpl k x from rfl,
      hk, key]
    rfl
...
private theorem realizable_of_implements_steps_eq_zero
  ...
  have hout : ∀ x, (A.M n).output ((A.M n).init x) = some (f n x) := by
    ...
    have key := PFunctor.DynSystem.DynComputation.ImplementsWithin.runWithInput_eq h
      (m := Id) (OracleHandler.ofFn (fun _ => true)).toQueryImpl x
    exact ((A.M n).runWith_eq_output_iterate_stepD (OracleHandler.ofFn (fun _ => true)) 0
      ((A.M n).init x)).symm.trans key
...
private theorem exists_diagonal_realizable :
  ...
  have hcard : ∀ n, (S n).card ≤ Cslib.Turing.SingleTapeTM.B (2 ^ (n / 4)) ^ 2 := fun n =>
