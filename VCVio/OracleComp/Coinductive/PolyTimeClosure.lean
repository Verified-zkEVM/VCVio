...
theorem precomp_implements {D : MachineAdversary bd} (f : (n : ℕ) → γ n → α n)
  ...
  intro n
  refine OracleMachine.Implements.of_runK_eq fun m _ _ H x => ?_
  rw [precomp_steps, precomp_M]
  exact (OracleMachine.runK_setInit (D.M n) _ H _ _).trans ((h n).runK_eq H (f n x))
...
theorem mapComp_implements {D : MachineAdversary bd} (g : (n : ℕ) → β n → γ n)
  ...
  intro n
  refine OracleMachine.Implements.of_runK_eq fun m _ _ H x => ?_
  rw [mapComp_steps, mapComp_M, OracleMachine.runK_setOutput, (h n).runK_eq H x]
