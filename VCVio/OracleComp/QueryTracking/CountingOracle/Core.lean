...
/-!
# Query counting and cost instrumentation
...
@[expose] def withCost (so : QueryImpl spec m) (costFn : spec.Domain → ω) :
...
@[expose] def costOracle {ω : Type u} [Monoid ω] (costFn : spec.Domain → ω) :
