...
theorem relTriple_simulateQ_run_writerT_of_impl_eq
  ...
  refine relTriple_post_mono
    (relTriple_simulateQ_run_writerT (spec₁ := spec₁) (spec₂ := spec₁)
      impl₁ impl₂ (fun (w₁ w₂ : ω) => w₁ = w₂) rfl ?_ oa ?_)
    (fun _ _ h => Prod.ext_iff.mpr h)
...
theorem tvDist_simulateQ_le_probEvent_bad
  ...
  have h_map :
    ...
    simpa [sim₁, sim₂, StateT.run', StateT.run] using
      (tvDist_map_le (m := OracleComp spec) (α := α × σ) (β := α) (fun x => x.1) sim₁ sim₂)
...
theorem tvDist_simulateQ_le_probEvent_output_bad
  ...
  have h_map :
      tvDist ((simulateQ impl₁ oa).run' (s₀, false))
        ...
        ≤ tvDist sim₁ sim₂ := by
    simpa [sim₁, sim₂, StateT.run', StateT.run] using
      tvDist_map_le (m := OracleComp spec) (α := α × σ × Bool) (β := α) (fun x => x.1) sim₁ sim₂
  exact h_map.trans <|
...
theorem tvDist_simulateQ_le_qeps_plus_probEvent_output_bad
  ...
  simpa [StateT.run', StateT.run] using
    (tvDist_map_le (m := OracleComp spec') (α := α × σ × Bool) (β := α) (fun x => x.1)
