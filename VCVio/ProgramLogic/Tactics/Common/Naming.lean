...
/-- Check that a binder name is neither anonymous nor generated by macro hygiene. -/
def isUsableBinderName (name : Name) : Bool :=
  !name.isAnonymous && !name.hasMacroScopes

/-- Prefix a bound-value name with `h` to name its support hypothesis. -/
...
/-- Choose a name based on the requested stem that is unused in the local context. -/
...
/-- Make suggested names distinct from the local context and from each other. -/
...
/-- Read a usable name from the outer lambda or universal binder. -/
...
/-- Read the continuation binder name of a monadic bind. -/
...
/-- Suggest a bound-value name from the current triple or probability goal. -/
...
/-- Choose fresh introduction names from goal binders and the computation's continuation. -/
...
/-- Choose fresh names for a probability congruence step and its optional support hypothesis. -/
...
/-- Choose fresh names for both bound values and their relational hypothesis. -/
...
/-- Introduce as many of the requested names as possible on the main goal. -/
...
/-- Try the requested introductions on every current goal. -/
...
/-- Try to assign the requested names to inaccessible binders in every goal. -/
def renameInaccessibleNames (names : Array Name) : TacticM Unit := do
  for name in names do
    discard <| tryEvalTacticSyntax
      (← `(tactic| all_goals first | rename_i $(mkIdent name):ident | skip))

/-- Render nonempty binder names as a tactic `as` clause. -/
