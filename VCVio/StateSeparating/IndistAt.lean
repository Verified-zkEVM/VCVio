...
def IndistAt {σ₀ σ₁ : Type}
    ...
    (h₁ : QueryImpl.Stateful unifSpec E σ₁) (s₁ : σ₁) (ε : ℝ≥0∞) : Prop :=
...
protected theorem symm
    {h₀ : QueryImpl.Stateful unifSpec E σ₀} {s₀ : σ₀}
    {h₁ : QueryImpl.Stateful unifSpec E σ₁} {s₁ : σ₁} {ε : ℝ≥0∞}
...
protected theorem trans
    {h₀ : QueryImpl.Stateful unifSpec E σ₀} {s₀ : σ₀}
    {h₁ : QueryImpl.Stateful unifSpec E σ₁} {s₁ : σ₁}
    {h₂ : QueryImpl.Stateful unifSpec E σ₂} {s₂ : σ₂} {ε₀ ε₁ : ℝ≥0∞}
...
theorem hybrid {n : ℕ} {σ : ℕ → Type} {ε : ℕ → ℝ≥0∞}
...
theorem link_inner_congr (outer : QueryImpl.Stateful M E σ_P) (sP : σ_P)
    ...
    {inner₁ : QueryImpl.Stateful unifSpec M σ₁} {s₁ : σ₁} {ε : ℝ≥0∞}
