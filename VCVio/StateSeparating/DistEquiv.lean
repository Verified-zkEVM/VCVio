...
theorem run_evalSPMF_eq
    {h₀ : QueryImpl.Stateful I E σ₀} {s₀ : σ₀}
    {h₁ : QueryImpl.Stateful I E σ₁} {s₁ : σ₁}
    (h : (h₀, s₀) ≡ᵈ (h₁, s₁)) {α : Type} (A : OracleComp E α) :
    𝒮[h₀.run s₀ A] = 𝒮[h₁.run s₁ A] := h A

theorem run₀_evalSPMF_eq
