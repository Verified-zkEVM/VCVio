...
theorem preserves_read (r s : CellRef Ident) :
  ...
  obtain rfl : z = (r.get h, h) := by simpa [read, Id.support_eq_singleton, Id.run] using hz
  simp
...
theorem write_writesOnly_single [DecidableEq Ident] (r : CellRef Ident) (x : r.Value) :
  ...
  intro s hs h z hz
  obtain rfl : z = (PUnit.unit, r.set h x) := by
    simpa [write, Id.support_eq_singleton, Id.run] using hz
  simpa using get_set_of_ne r s h x hs
...
theorem demoImpl_writesOnly :
  ...
  cases t with
  | touchLog =>
      exact CellRef.writeM_supportWritesOnly_single logRef 1
  | touchCache =>
      exact CellRef.writeM_supportWritesOnly_single cacheRef 1
  | readFlag =>
      show CellRef.SupportWritesOnly (α := PUnit)
        (demoImpl DemoQuery.readFlag) (demoWrites DemoQuery.readFlag)
      have h : CellRef.SupportWritesOnly (α := PUnit)
          (demoImpl DemoQuery.readFlag) (∅ ∪ ∅ : Set DemoCell) :=
