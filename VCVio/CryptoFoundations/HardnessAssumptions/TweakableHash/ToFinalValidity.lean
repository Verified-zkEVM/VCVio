...
source-final-validity game with *the same* success probability. Each game contributes
`Problem.toSourceFinalValidity`, `Adversary.toSourceFinalValidity`, and the three theorems
`..._experiment_toSourceFinalValidity`, `..._advantage_toSourceFinalValidity` and
`..._advantage_le_toSourceFinalValidity`. `SM_DT_TCR_*` is covered here; the same construction
applies to any further game sharing this shape.
...
variable {ι PkSeed Tweak M Y Q : Type}
