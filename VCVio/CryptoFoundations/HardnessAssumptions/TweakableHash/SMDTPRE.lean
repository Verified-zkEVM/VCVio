...
abbrev problem : SM_DT_PRE_Problem Unit Unit Bool Bool Bool Bool where
  th := hash
  emb := id
...
def queries : OracleComp (SM_DT_PRE_challengeSpec Bool Bool + collectionSpec coll) Unit := do
  let _ ← (SM_DT_PRE_challengeSpec Bool Bool + collectionSpec coll).query (.inl false)
  let _ ← (SM_DT_PRE_challengeSpec Bool Bool + collectionSpec coll).query (.inr ⟨(), false, true⟩)
  let _ ← (SM_DT_PRE_challengeSpec Bool Bool + collectionSpec coll).query (.inl true)
  return ()
...
theorem history_in_issue_order :
    ∀ p ∈ support ((simulateQ (SM_DT_PRE_oracles problem ()) queries).run ([], [])),
      p.2.1.map Prod.fst = [false, true] ∧ p.2.2 = [] := by
  simp [queries, SM_DT_PRE_oracles, SM_DT_PRE_challengeOracle, collectionOracle]
  grind
