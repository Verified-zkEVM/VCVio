...
(`MachineAdversary`, `OracleComp.IsPolyTime`) to the asymptotic security games of
...
secure against all polynomial-time families. This is where the certificate's derived
query bound (`PolyTimeWitness.queryBound`) and the `steps` polynomial do quantitative
work: the adversary's polynomially many queries turn per-query loss into
