...
rounds. The strongest theorem uses one global query bound for a proof-only runner that mirrors the
commitment phases and ends in a log-dependent accounting computation. A scheduled compatibility
theorem uses `reserveQueries` to recover the earlier sum-of-public-budgets interface.
-/
