...
functions, in Cslib's `Cslib.Turing.SingleTapeTM` model:
...
theorem size_constPolyTimeComputable_le (out : List Bool) :
  ...
  | nil =>
    ...
    simp
  | cons o os =>
    change Fintype.card (Unit ⊕ Fin (o :: os).length) ≤ (o :: os).length + 2
...
noncomputable def const {α : Type u} {β : Type v} (ea : α → List Bool)
  ...
  toFun _ := eb c
  polyTime := Cslib.Turing.SingleTapeTM.constPolyTimeComputable (eb c)
  map_encode _ := rfl
...
theorem size_ofFintype_le {α : Type u} {β : Type v} [Fintype α]
  ...
  have hinj : ∀ a ∈ Finset.univ, ∀ b ∈ Finset.univ, ea a = ea b → a = b :=
    fun a _ b _ h => hea h
  have h1 : (Cslib.Turing.SingleTapeTM.prefixClosure (Finset.univ.image ea)).card ≤
      ∑ a : α, ((ea a).length + 1) := by
    refine (Cslib.Turing.SingleTapeTM.card_prefixClosure_le _).trans (le_of_eq ?_)
