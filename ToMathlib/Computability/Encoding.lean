...
- `Computability.finEncodingOfFinEnum`: a `PackedEncoding` of any `FinEnum` type (unary
  over a `Unit` alphabet), making the common finite cases — `Unit`, `Bool`, `Fin n`,
  products, sums, sigmas — encodable for free.
- `Computability.finEncodingOption`: a `PackedEncoding` of `Option β` from one of `β`.
- `Computability.finEncodingSigma`: a `PackedEncoding` of a dependent pair `(t : ι) × F t`
  from an encoding of the index and per-index encodings over a shared fiber alphabet.
- `Computability.PackedEncoding.boolify`: relabel any finite-alphabet encoding into
...
These mirror the design of `Computability.encodingProd`.
-/
...
/-! ## Bundled Finite-Alphabet Encodings -/

/-- A finite-alphabet encoding with the alphabet **bundled** as a field.

Mathlib's `Computability.Encoding` takes the alphabet as a type parameter; this
Σ-style packaging remains necessary here because the polynomial-time layer
manipulates encodings of *varying* alphabet — per-`FinEnum` unary alphabets, sum
alphabets for options and sigmas — before `PackedEncoding.boolify` normalizes
them all to `Bool`. Local to `ToMathlib`. -/
structure PackedEncoding (α : Type u) where
  /-- The finite tape alphabet of the encoding. -/
  Γ : Type
  /-- The encoding function. -/
  encode : α → List Γ
  /-- The decoding function. -/
  decode : List Γ → Option α
  /-- Decoding is a retraction of encoding. -/
  decode_encode : ∀ x, decode (encode x) = some x
  /-- The alphabet is finite. -/
  ΓFin : Fintype Γ

attribute [instance] PackedEncoding.ΓFin
attribute [simp] PackedEncoding.decode_encode

namespace PackedEncoding

variable {α : Type u} (e : PackedEncoding α)

/-- The underlying unbundled Mathlib encoding. -/
def toEncoding : Encoding α e.Γ where
  encode := e.encode
  decode := e.decode
  decode_encode := e.decode_encode

@[simp] theorem toEncoding_encode (x : α) : e.toEncoding.encode x = e.encode x := rfl

/-- Packed pair encoding over the sum alphabet: the bundled form of
`Computability.encodingProd`. -/
def pair {β : Type v} (eb : PackedEncoding β) : PackedEncoding (α × β) where
  Γ := e.Γ ⊕ eb.Γ
  encode x := (e.encode x.1).map .inl ++ (eb.encode x.2).map .inr
  decode x := Option.map₂ Prod.mk (e.decode (x.filterMap Sum.getLeft?))
...
/-- A `PackedEncoding` of any `FinEnum` type: unary encoding over the `Unit` alphabet,
...
def finEncodingOfFinEnum (α : Type u) [FinEnum α] : PackedEncoding α where
  Γ := Unit
...
/-- A `PackedEncoding` of `Option β`: `none` is the empty string, `some b` is a marker
symbol followed by the relabeled encoding of `b`. -/
def finEncodingOption {β : Type u} (eb : PackedEncoding β) : PackedEncoding (Option β) where
  Γ := Unit ⊕ eb.Γ
  encode
    | none => []
...
/-- Sigma analogue of `PackedEncoding.pair`: encode the index with `.inl` symbols and the
...
`PackedEncoding`s for the same reason. -/
def finEncodingSigma {ι : Type u} (ei : PackedEncoding ι) {F : ι → Type v} {Γ : Type}
    [Fintype Γ] (enc : (t : ι) → F t → List Γ) (dec : (t : ι) → List Γ → Option (F t))
    (henc : ∀ t x, dec t (enc t x) = some x) : PackedEncoding ((t : ι) × F t) where
  Γ := ei.Γ ⊕ Γ
...
consume and produce values of any `PackedEncoding`-encodable type. -/
...
theorem length_boolify_finEncodingOfFinEnum {γ : Type u} [FinEnum γ] [Fintype γ] (x : γ) :
  ...
  rw [PackedEncoding.length_boolify]
...
def finEncodingBitVec (w : ℕ) : PackedEncoding (BitVec w) where
  Γ := Bool
...
theorem length_boolify_finEncodingBitVec {w : ℕ} (m : BitVec w) :
  ...
  rw [PackedEncoding.length_boolify, length_encode_finEncodingBitVec]
  change (Fintype.card Bool + 1) * w = 3 * w
  rw [Fintype.card_bool]

/-! ## Encoding Lengths of Pairs and Options -/

theorem length_encode_pair {α : Type u} {β : Type v} (ea : PackedEncoding α)
    (eb : PackedEncoding β) (x : α × β) :
    ((ea.pair eb).encode x).length =
      (ea.encode x.1).length + (eb.encode x.2).length := by
  simp [PackedEncoding.pair]

@[simp] theorem length_encode_finEncodingOption_none {β : Type u} (eb : PackedEncoding β) :
    ((finEncodingOption eb).encode (none : Option β)).length = 0 := rfl

theorem length_encode_finEncodingOption_some {β : Type u} (eb : PackedEncoding β) (b : β) :
...
theorem length_boolify_pair {α : Type u} {β : Type v} (ea : PackedEncoding α)
    (eb : PackedEncoding β) (x : α × β) :
    ((ea.pair eb).boolify x).length
      = (Fintype.card ea.Γ + Fintype.card eb.Γ + 1)
          * ((ea.encode x.1).length + (eb.encode x.2).length) := by
  have hc : Fintype.card (ea.pair eb).Γ = Fintype.card ea.Γ + Fintype.card eb.Γ :=
    Fintype.card_sum
  rw [PackedEncoding.length_boolify, length_encode_pair, hc]
...
theorem length_boolify_finEncodingOption {β : Type v} (eb : PackedEncoding β) (x : Option β) :
  ...
  rw [PackedEncoding.length_boolify, hc]
...
`M₁.State ⊕ M₂.State`. -/
def finEncodingSum {α : Type u} {β : Type v} (ea : PackedEncoding α) (eb : PackedEncoding β) :
    PackedEncoding (α ⊕ β) where
  Γ := Bool ⊕ ea.Γ ⊕ eb.Γ
  encode
...
@[simp] theorem length_encode_finEncodingSum_inl {α : Type u} {β : Type v} (ea : PackedEncoding α)
    (eb : PackedEncoding β) (a : α) :
    ((finEncodingSum ea eb).encode (Sum.inl a : α ⊕ β)).length = (ea.encode a).length + 1 := by
  simp [finEncodingSum]

@[simp] theorem length_encode_finEncodingSum_inr {α : Type u} {β : Type v} (ea : PackedEncoding α)
    (eb : PackedEncoding β) (b : β) :
...
theorem length_boolify_finEncodingSum {α : Type u} {β : Type v} (ea : PackedEncoding α)
    (eb : PackedEncoding β) (x : α ⊕ β) :
