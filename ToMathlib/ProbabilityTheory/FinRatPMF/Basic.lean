...
Finite rational distributions provide executable semantics for computations with finite
...
Their measure interpretation is provided in `ToMathlib.ProbabilityTheory.FinRatPMF.Measure`.
-/
...
@[simp] lemma prob_pure [DecidableEq α] (a x : α) :
    (pure a : Raw α).prob x = if x = a then 1 else 0 := by
  simp only [Raw.prob, probOfList, pure_toList]
  by_cases h : x = a
  · simp [h]
...
/-- Binding a raw quotient value chooses a representative for each continuation. -/
lemma bind_mk (p : Raw α) (f : α → FinRatPMF β) :
    (mk p >>= f) = mk (Raw.bind p (fun a => Quotient.out (f a))) := rfl

/-- Choosing a representative and forming its distribution recovers the quotient value. -/
@[simp] lemma mk_out (p : FinRatPMF α) : mk (Quotient.out p) = p :=
  Quotient.out_eq p

/-- The quotient constructor preserves bind with raw continuations. -/
@[simp] lemma bind_mk_mk (p : Raw α) (f : α → Raw β) :
    (mk p >>= fun a => mk (f a)) = mk (p >>= f) := by
  rw [bind_mk]
  apply Quotient.sound
  exact SameDist.bind_congr (.refl p) fun a => Quotient.exact (mk_out (mk (f a)))
...
noncomputable instance : LawfulMonad FinRatPMF := LawfulMonad.mk'
  (id_map := fun x => by
    ...
    change (mk p >>= fun a => mk (pure a)) = mk p
    rw [bind_mk_mk, bind_pure])
  (pure_bind := fun x f => by
    change mk ((pure x : Raw _) >>= fun a => Quotient.out (f a)) = f x
    rw [pure_bind, mk_out])
  (bind_assoc := fun m f g => by
    refine Quotient.inductionOn m fun p => ?_
    change (mk p >>= f >>= g) = (mk p >>= fun a => f a >>= g)
    rw [bind_mk, bind_mk, bind_mk]
    apply Quotient.sound
    change SameDist
      ((p >>= fun a => Quotient.out (f a)) >>= fun b => Quotient.out (g b))
      (p >>= fun a => Quotient.out (f a >>= g))
    rw [bind_assoc]
    exact SameDist.bind_congr (.refl p) fun a => (out_bind_sameDist (f a) g).symm)
