...
- Turing-machine-grounded polynomial-time adversaries (`PolyTimeAdversary`, `OracleComp.IsPolyTime` for the `secureAgainst` `isPPT` slot, the `PolyQueries` bridge, and the total-run-time bound `detTotalTime_le`): `VCVio/OracleComp/Coinductive/PolyTime.lean`; game-level transfer in `VCVio/CryptoFoundations/Asymptotics/PolyTime.lean`; worked examples in `Examples/DynamicalSystems/PolyTime.lean` (incl. the adaptive two-oracle machine `adaptCoins`) and `Examples/DynamicalSystems/XorFlips.lean` (the n-flip XOR family: the first fully concrete, hypothesis-free `IsPolyTime` instance, plus game wiring). Machine witnesses are `Computability.EncPolyTime` (`ToMathlib/Computability/CslibPolyTime.lean`), grounded in the Cslib dependency's `Turing.SingleTapeTM.PolyTimeComputable` (proven composition); encoding combinators in `ToMathlib/Computability/Encoding.lean`, verified base machines in `ToMathlib/Computability/PolyTimeTM.lean` — incl. the finite-table machine `tableComputer` giving `EncPolyTime.ofFintype` (any function on a finite domain is polytime computable relative to an injective encoding, discharging all four per-step witnesses of finite-state machines)
