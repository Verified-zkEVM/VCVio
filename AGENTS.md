...
- Oracle strategies (the coalgebra dual: `OracleStrategy`, `OracleHandler := PFunctor.Section` with `ofFn`/`DFunLike`/`Coe`, run/transcript, headline `iterate_advance_eq_simulate`): `VCVio/OracleComp/Coinductive/DynSystem.lean`, with worked examples in `Examples/DynamicalSystems/Basic.lean`
- Oracle machines (strategies with init/readout: `OracleMachine`, early-stopping runs `runD`/`runK`, the probabilistic `Implements` relation with the `IsSimulation` proof method, and the program ↔ machine bridges `OracleComp.toMachine` / `OracleMachine.toComp` identifying query bounds with steadiness): `VCVio/OracleComp/Coinductive/Machine.lean`
- Turing-machine-grounded polynomial-time adversaries (`MachineAdversary bd`, the certificate `PolyTimeWitness bd oa`, and `OracleComp.IsPolyTime bd` for the `secureAgainst` `isPPT` slot): `VCVio/OracleComp/Coinductive/PolyTime.lean` — the model is non-uniform **P/poly relative to pinned canonical boundary encodings** `bd : BoundaryData` (fixed-width `Computability.BitEncFam` for inputs/outputs/interface; the width bound is the formalized Katz–Lindell `1^n` convention; existential encodings would let `enc x := std x ++ f x` cache any function — see the `## Model` docstring and `docs/agents/polytime-model.md`). Machine-internal state uses variable-width `Computability.StrEncFam`; the four step witnesses are `Computability.EncPolyTimeFam` (uniform machine families with one time and one advice polynomial each, `ToMathlib/Computability/BitEncoding.lean`), grounded in Cslib's proven `Turing.SingleTapeTM.PolyTimeComputable`. Closures at `VCVio/OracleComp/Coinductive/PolyTimeClosure.lean` (`precomp`, `mapComp`, abstract `IsPolyTime.map` — derivable exactly because boundaries are canonical); game transfer in `VCVio/CryptoFoundations/Asymptotics/PolyTime.lean` (incl. the per-query-loss former); hypothesis-free worked example `Examples/DynamicalSystems/XorFlips.lean`; superpolynomial-accumulator constructions (`uniformBitVec`, the KL sampler) in honest hypothesis form pending a base-machine library; non-triviality acceptance targets (documented sorries) in `VCVio/OracleComp/Coinductive/PolyTimeNontrivial.lean`
- Query logs/counts unified with PolyFun traces (`QueryLog = PFunctor.TraceList`, `QueryCount = Control.Trace ℕ`, free-monoid `FreeMonoid.lift` readouts): `VCVio/OracleComp/QueryTracking/Trace.lean`
