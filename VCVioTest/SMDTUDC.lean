...
namespace SMDTUDCSourceFinalValidityTest
...
def problem : SM_DT_UD_C_SourceFinalValidity.Problem Unit Seed Bool Bool Bool where
  th := hash
  inputGen := pure false
  outputGen := pure true
  thColl := collection
...
abbrev Specs := unifSpec +
  (SM_DT_UD_C_SourceFinalValidity.challengeSpec Bool Bool +
...
def separate : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool
  pick := challenge false
  distinguish y _ := pure (!y)

/-- The reverse distinguisher pins the other direction of the symmetric advantage. -/
def separateReverse : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool
  pick := challenge false
  distinguish y _ := pure y

/-- The second distinct target exceeds the cap, but its answer is still returned. -/
def exceedCap : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool × Bool
  pick := do
...
def duplicateTarget : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool × Bool
  pick := do
    let y₁ ← challenge false
    let y₂ ← challenge false
...
def crossClash : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool × Bool
  pick := do
    let y₁ ← challenge false
...
def repeatCollection : SM_DT_UD_C_SourceFinalValidity.Adversary problem where
  State := Bool × Bool
  pick := do
...
private lemma run_separate_real :
    (simulateQ (SM_DT_UD_C_SourceFinalValidity.oracles .real problem .only)
      separate.pick).run .initial =
      pure (false, ⟨[false], [], true⟩) := by
  rfl

private lemma run_separate_ideal :
    (simulateQ (SM_DT_UD_C_SourceFinalValidity.oracles .ideal problem .only)
      separate.pick).run .initial =
...
private lemma experiment_separate_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real separate = pure true := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_separate_real]
  rfl

private lemma experiment_separate_ideal :
    SM_DT_UD_C_SourceFinalValidity.Experiment .ideal separate = pure false := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_separate_ideal]
  rfl

private lemma experiment_separateReverse_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real separateReverse = pure false := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_separateReverse_real]
  rfl

private lemma experiment_separateReverse_ideal :
    SM_DT_UD_C_SourceFinalValidity.Experiment .ideal separateReverse = pure true := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_separateReverse_ideal]
  rfl

private lemma experiment_exceedCap_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real exceedCap = pure false := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_exceedCap_real]
  rfl

private lemma experiment_duplicateTarget_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real duplicateTarget = pure false := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_duplicateTarget_real]
  rfl

private lemma experiment_crossClash_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real crossClash = pure false := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_crossClash_real]
  rfl

private lemma experiment_repeatCollection_real :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real repeatCollection = pure true := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
  rw [run_repeatCollection_real]
  rfl

private lemma experiment_repeatCollection_ideal :
    SM_DT_UD_C_SourceFinalValidity.Experiment .ideal repeatCollection = pure true := by
  simp only [SM_DT_UD_C_SourceFinalValidity.Experiment, problem_seedGen, pure_bind]
...
theorem real_ideal_separation_canary :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real separate = pure true ∧
      SM_DT_UD_C_SourceFinalValidity.Experiment .ideal separate = pure false ∧
      SM_DT_UD_C_SourceFinalValidity.RealSuccess separate = 1 ∧
      SM_DT_UD_C_SourceFinalValidity.IdealSuccess separate = 0 ∧
      SM_DT_UD_C_SourceFinalValidity.DirectedAdvantage separate = 1 ∧
      SM_DT_UD_C_SourceFinalValidity.AbsoluteAdvantage separate = 1 := by
  simp [experiment_separate_real, experiment_separate_ideal,
    SM_DT_UD_C_SourceFinalValidity.RealSuccess, SM_DT_UD_C_SourceFinalValidity.IdealSuccess,
    SM_DT_UD_C_SourceFinalValidity.DirectedAdvantage,
    SM_DT_UD_C_SourceFinalValidity.AbsoluteAdvantage, ENNReal.absDiff]
...
theorem source_orientation_reverse_canary :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real separateReverse = pure false ∧
      SM_DT_UD_C_SourceFinalValidity.Experiment .ideal separateReverse = pure true ∧
      SM_DT_UD_C_SourceFinalValidity.RealSuccess separateReverse = 0 ∧
      SM_DT_UD_C_SourceFinalValidity.IdealSuccess separateReverse = 1 ∧
      SM_DT_UD_C_SourceFinalValidity.DirectedAdvantage separateReverse = -1 ∧
      SM_DT_UD_C_SourceFinalValidity.AbsoluteAdvantage separateReverse = 1 := by
...
theorem final_validity_poison_canary :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real exceedCap = pure false ∧
      SM_DT_UD_C_SourceFinalValidity.Experiment .real duplicateTarget = pure false ∧
      SM_DT_UD_C_SourceFinalValidity.Experiment .real crossClash = pure false := by
...
theorem repeated_collection_allowed_canary :
    SM_DT_UD_C_SourceFinalValidity.Experiment .real repeatCollection = pure true ∧
      SM_DT_UD_C_SourceFinalValidity.Experiment .ideal repeatCollection = pure true := by
...
end SMDTUDCSourceFinalValidityTest
