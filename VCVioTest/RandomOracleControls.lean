...
Free local coins and charged hash requests use different branches of one oracle sum. The
existing `unifFwdImpl` forwards local coins, and the existing `randomOracle` caches hash
answers. Adversaries receive only the public address; the experiment owns the cache.
...
/-- Exactly the hash branch consumes the query allowance. -/
@[expose]
def IsHashQuery : Nat ⊕ Bool → Prop
  | .inl _ => False
  | .inr _ => True

instance : DecidablePred IsHashQuery
  | .inl _ => isFalse not_false
...
abbrev IsHashQueryBound {α : Type} (oa : OracleComp MainSpec α) (q : ℕ) : Prop :=
  oa.IsQueryBoundP IsHashQuery q
...
def oneQuery : QueryGapAdversary 1 where
  ...
  bound _ := by simp [IsHashQueryBound, IsHashQuery]
...
theorem oneQuery_not_zero (t : Bool) : ¬ IsHashQueryBound (oneQuery.attack t) 0 := by
  simp [oneQuery, IsHashQueryBound, IsHashQuery]
...
theorem localCoin_zero_bound (n : ℕ) :
  ...
  simp [IsHashQueryBound, IsHashQuery]
...
private theorem finishGuess_localCoin (n : ℕ)
    (k : Fin (n + 1) → OracleComp MainSpec Bool) (t : Bool) :
    runROM (finishGuess (liftM (MainSpec.query (.inl n)) >>= k) t) =
      ($[0..n] : ProbComp (Fin (n + 1))) >>= fun x ↦ runROM (finishGuess (k x) t) := by
  simp [finishGuess, runROM, unifFwdImpl, StateT.run'_eq, ProbComp.uniformFin_def]
...
theorem zeroHashQuery_guess_probability (attack : OracleComp MainSpec Bool)
  ...
  induction attack using OracleComp.inductionOn with
  | pure guess =>
    -- With no query, the checked cell is a fresh bit independent of the fixed guess.
    have hrun : runROM (finishGuess (pure guess) t) =
        (do let b ← $ᵗ Bool; pure (decide (guess = b)) : ProbComp Bool) := by
      simp [finishGuess, runROM, StateT.run'_eq]
    rw [hrun]
    exact uniform_bool_guess guess
  | query_bind request k ih =>
    change OracleComp.IsQueryBoundP (liftM (MainSpec.query request) >>= k) IsHashQuery 0 at h
    rw [OracleComp.isQueryBoundP_query_bind_iff] at h
    cases request with
    | inr _ =>
      -- A charged hash request is impossible under the zero bound.
      simp [IsHashQuery] at h
    | inl n =>
      -- A free local draw preserves the empty cache; average the inductive success law.
      have hk : ∀ x, IsHashQueryBound (k x) 0 := by simpa [IsHashQuery] using h.2
      rw [finishGuess_localCoin]
      rw [evalDist_bind_of_discrete,
        Measure.bind_apply (measurableSet_singleton true) Measurable.of_discrete.aemeasurable]
      simp only [fun x ↦ ih x (hk x), lintegral_const,
