...
An adaptive tag/reader client instantiates both output polarities, their concrete query
...
example (flip : Bool) :
    IsQueryBoundP (unlinkToMultiplePRFReduction (sessionsPerTag := 2)
      (polarity adaptive flip)) (·.isRight) 3 ∧
    IsQueryBoundP (unlinkToSinglePRFReduction (sessionsPerTag := 2)
      (polarity adaptive flip)) (·.isRight) 5 := by
  simpa using named_reduction_budgets adaptive 1 1 reader_bound tag_bound flip

-- The collision, multiple reader, aliasing, and single reader losses are 8, 2, 1, and 4
-- sixty-fourths, respectively. The explicit PRF hypotheses name all four distinguishers.
...
example {K : Type} (prfs : TagReaderPRFs K Bool (Fin 64) (Fin 64) 2) :
    gap prfs 2 (polarity adaptive true) = -gap prfs 2 adaptive :=
  gap_not prfs 2 adaptive (Network.totalQueryBound adaptive 1 1 reader_bound tag_bound)
