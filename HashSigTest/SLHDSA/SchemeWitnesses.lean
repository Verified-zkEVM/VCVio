...
-- := id` is checked against `Bytes 1 → Bytes toyParams.n`.  `toy` is exposed because everything
-- after the bundle is written at `toy`, so `toy.params` has to reduce to `toyParams` for
-- `toyPrimitives : Primitives toyParams` to be accepted where `Primitives toy.params` is expected.
-- Neither the secret map, the tweak map, the randomizer, the digest map nor any of the four
-- positions needs exposure of its own.  Nothing outside this executable consumes them.
...
`Security.ReachableTargets` membership lemma applies to the advanced position unchanged. -/
