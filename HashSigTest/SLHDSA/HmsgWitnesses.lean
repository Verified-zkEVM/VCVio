...
which is `checkFixture` here and `checkToyBundle` there.  Strip the comments, the docstrings and
that one dropped attribute, and the two blocks differ in a single string: the failure message.
...
* *That `globalLeaf` is the local leaf.*  Rejected six times over, each of the six sufficient
  ...
  typechecks moves nine declarations, and is then refused by each of those six.  The *per-index*
  read of that `Adrs` table is not one of them: the shifted global leaf stands on both sides of its
  comparison, so with it alone the shift passes.  It pins how `forsNodeAdrs` builds an address from
  a given global leaf, not which global leaf.  The `Nodup` sweep over the sixty-four indices lets
  the shift through for its own reason: a shift merely permutes those sixty-four leaves, so they
  stay distinct.
...
-- neither do the secret map, the tweak map, the randomizer or the digest map.  Nothing outside this
...
`pkRoot` below, which passes it to `GeneralHypertree.root`, and the `BottomPosition.ofDigestParts`
pin, which is an `example`.  Neither needs its body. -/
...
anyway, because it is the law about `forsNodeAdrs` quantified over the index that a reader checks
the imported function against, rather than a statement about these two values. -/
