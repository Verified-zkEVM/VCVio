...
stand in.** A check implied by the *conjunction* of others still fires first on the edit it is named
for and names the cell that moved, which a check implied by conjuncts that all precede it cannot do
at all. Both classes hold over the `2 ^ 12` ways of dropping roles from the table — 512 distinct
role columns, each counted eight times because the three roleless rows have nothing to drop:
...
and a public seed and no security assumption at all, and the bound it names is at least one. Applied
here, that says the corollaries of `HashSig.SLHDSA.Security.LimitedProfile` are no stronger than
what they instantiate: `limitedAdvantage_le_bound` at the certificate below reads `adv.advantage ≤
(something ≥ 1)`, which `probOutput_le_one` gives with extra steps, and the two
