...
* **A paired edit of the `w − 2` coefficient — though not the one the general fixture
  ...
  equation, in the two written-out corollaries, or in all three at once is refused by the library
  module itself, in every combination.  What is silent is the *general* edit
  `HashSigTest.SLHDSA.Composition` records:
...
cannot do at all.  Both classes hold over the `2 ^ 12` ways of dropping roles from the table —
512 distinct role columns, each counted eight times because the three roleless rows have nothing
to drop:
...
* two predicates are absent because no edit could reach them.  "The FORS-F role is the only one
  used twice" is "twelve summands" and "three summands have no cap role" restated, both of which
  precede it: of the 4096 masks none satisfies those two and falsifies it, and the two edits that
  falsify it — `forsFTcr` losing its role, cap and arity, and `hmsgItsr` gaining all three — both
  stop at "three summands have no cap role".  "Every cap is positive" is implied by "every role
  row carries its own cap" together with the fact that all eight of this profile's caps are
  positive, so `wotsTlTcr`'s cap at `0` stops one check earlier; it would be worth having at a
...
-- `noncomputable`, because without it this pin is itself a compiled constant and the build
...
substitution matrix.**  Each of the eight directly-proved statements re-proved by each of the
eight equations is sixty-four elaborations, of which forty-eight are refused and sixteen
accepted.  The refusals are every cross-arity cell — the WOTS+-`F` undetectability statement
re-made about the arity-two FORS-`H` game, the FORS-`H` statement made at the single-node type —
and they include the standalone/collection boundary, in both directions between `forsFOpenPre`
and any WOTS+-`F` collection game.  Of the sixteen accepted, eight are
...
the nine carrier instances `HashSig.SLHDSA.Security.LimitedProfile` declares, and those are the
part a concrete profile adds.  Restating it here catches an edit that breaks the concrete case
alone, exactly as
