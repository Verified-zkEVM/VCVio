...
import all Extern.Falcon.Expm.Polynomial
...
`mtwop63` is the conversion of an `FPR` in `[0, 1)` to a `63`-bit fixed-point fraction. Its
result is `expm_p63_sub_trueArg_le`. The second half — certifying the coefficient table against
`Real.exp` — is a numerical-certification problem of a different character, in
`Extern.Falcon.Expm.Certificates` and described in `docs/agents/expm-certification.md`.

The module reads only `Extern.Falcon.FPR.Decode` of the FPR bridge, never the arithmetic proofs,
so it compiles in parallel with them. It ends with the sign-blindness of `expm_p63`.
-/
...
theorem expm_p63_sub_trueArg_le (x ccs : FPR)
    (h0 : 0 ≤ toReal x) (hub : toReal x ≤ 694 / 1000)
