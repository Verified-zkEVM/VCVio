...
/-- The bound at the SP 800-230 reduced profile: nothing about the corollaries is runnable, because
each instantiates a statement about a probability and every probability in them is
...
expression, its game's cap role, that cap at this profile checked against `targetCount` rather
than only written down, and the arity of the hash the game attacks.  Two cells of that table go
dark at this profile and both are asserted rather than hidden: the two `T_l` compressions have the
same cap at every one-layer parameter set and are separated only by their arity, six against
sixty-eight, and the WOTS+-`F` undetectability and preimage roles have the same cap at *every*
parameter set and the same arity too, so only their games' types separate them — which the pins
read off the two certificate fields.  Everything about the corollaries' own shape — the nine
carrier instances none of which instance search finds at this bundle, the five carriers they are
stated at, both coefficients as numerals, the ten games' caps read at the concrete bundle, the two
`T_l` games' attacked-member equations at their own arities, and each of the five exported
corollaries — is pinned by elaboration, at least one `example` per exported declaration, in a file
no library-side edit can reach.  So is their strength: the vacuity canary rebuilds the composition
fixture's free certificate at this bundle and proves that at it both headlines bound the advantage
by something at least one. -/
