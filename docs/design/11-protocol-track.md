...
**IPDL** (Gancher–Sojakova–Fan–Shi–Morrisett, POPL 2023; ePrint 2021/147): protocols as systems
of typed channels with reactive assignments; security = *approximate observational equivalence*
proved in an equational logic whose rules are congruence + channel-algebra laws + assumption
application; mechanized in Rocq; case studies (coin toss, OT, GMW) at line counts far below
EasyUC's ≈18k. The reading: IPDL processes are open-system presentations, and its equational
theory is a hand-picked fragment of what `OpenTheory` + `Emulates`-congruence + the module laws
provide. IPDL works because equivalence is a congruence for its composition operators *by
construction of the logic* — which is precisely what the behavior model buys us as theorems
(`02` §3.4). IPDL's known boundaries (fixed channel topology, bounded replication, no state
beyond channel contents) are boundaries `06` is built to pass. Adoption: not the calculus — the
*proof-size discipline*. Every hop in an IPDL proof is one equation; our hop engine should make
VCVio proofs read the same way. IPDL's published line counts are the honest baseline for R-11.2.
...
the computational model — label lattice with an adversary label, nonce/key indexing, typing
implies security against probabilistic poly-time adversaries (soundness via a logical relation),
subprotocols typecheck separately and compose. OwlC then compiles typed protocols to verified,
interoperable Rust (WireGuard, HPKE) at industrial performance. The reading: **an Owl-style
label assignment is a display over the protocol's interaction boundary** — labels decorate
positions/directions, well-typedness is a displayed lift of the protocol program, and the
soundness theorem is a (one-per-lattice) bridge from the displayed lift to a simulation
statement. That makes Owl the third instance of `04`'s thesis (program logics are displayed
structure), alongside instrumentation and relational judgments. Adoption is deliberately staged:
this cycle only a recast memo + toy (a typed one-message flow whose displayed lift discharges
its secrecy hop); a real typed front-end is a paper-sized project. OwlC's extraction story is
...
assumptions apply only to keys whose secrecy survives the epoch analysis. Deliverables: an
...
2. **P-2 (IPDL parity):** one IPDL case study (coin toss or OT) as an `Emulates`-congruence
   equational proof over the behavior model. Line count vs. the published IPDL artifact.
...
- **R-11.2**: the P-2 case study lands within 2× the IPDL artifact's line count (honest
  yardstick: they are a special-purpose calculus; we are a general framework — if we exceed 2×,
  the equational surface of the behavior model is not yet usable and that finding goes in `02`'s
  honest column).
...
- **IPDL comparison caveat**: IPDL proofs are short partly because their adversary/simulator
  are *syntactically restricted*; our P-2 must state the exactly-equal theorem or note the
  quantifier difference (ground rule 3).
