...
| F-020 | HIGH | REMEDIATED-PENDING-REVIEW | Reports omitted the FIPS WOTS checksum-shift correction | Appendix A changes line 6 of sign/recovery because submission pseudocode can shift incorrectly when `lgw != 4` | S05 implements the exact outer-mod shift and byte pipeline and adds an `lgw = 2` regression distinguishing shift zero from erroneous shift eight; independent S05 review remains required |
