...
const BASELINE = typeof A.baseline === 'number' ? A.baseline : 14
...
const DRIFT = `RECORDED BASELINE (docs/agents/falcon-review.md §4, since s12): ${BASELINE} live sorrys total = 9 in LatticeCrypto/Falcon/ (NTRUSolver 2 = solve_NTRU_intermediate/solve_NTRU_depth0 ascent stubs ~366/384; FPRBridge 5 = per-op FPR error bounds; Scheme 1 = keyGenFromSeed; Security 1 = euf_cma_security) + 4 generic load-bearing at GPVHashAndSign.lean:270,283,332,363 + 1 at ToMathlib/.../RenyiDivergence.lean. NOTE: the 9 old NTRUSolver FXR/PolyBigInt stub-sorries were REMOVED in s12 (now import the real Concrete/FXR.lean + PolyBigInt.lean); verify_sign_correct + F8 are PROVEN (s10). ApproxArith.lean sorrys are comment-only (not live). If your reading deviates, raise a finding "drift" (severity per impact) noting the new file:line.`
...
const DIMENSIONS = [
  ...
  { key: 'KeyGen+NTRUSolve', files: 'Scheme.lean:89,112-121 (keyGenFromSeed, validKeyPair, ntruEquation); Concrete/KeyGen.lean; Concrete/NTRUSolver.lean; Concrete/{FXR,PolyBigInt,BigInt31,SmallPrimeNTT}.lean', focus: 'keyGenFromSeed sorry (Scheme.lean:121, ZERO callers — disconnected from Concrete KeyGen.keyGen); the 2 live NTRUSolver sorrys = solve_NTRU_intermediate (~366) + solve_NTRU_depth0 (~384) ascent stubs, so solve_NTRU is non-functional for n>1; the old FXR.sqr/vect_*/poly_big_to_small stubs were REPLACED s12 by real Concrete/FXR.lean + PolyBigInt.lean (open at NTRUSolver.lean:68-69) — confirm they are gone/not shadowed and check_ortho_norm is sorry-free; any proof of fG-gF=q (ntruEquation) or gamma bound (none expected); NOTE third_party/c-fn-dsa submodule may be uninitialized → kgen_*.c byte-faithfulness unverifiable.' },
  ...
  { key: 'Theorem-soundness', files: 'Security.lean (verify_sign_correct PROVEN s10, euf_cma_security:~382, ntruSISProblem, ntruPSFCollisionProblem, SamplerQuality, HasUniformSamplerLoss); VCVio/CryptoFoundations/GPVHashAndSign.lean (Correct:91, reductions:270/283, forgery lemmas:332/363, euf_cma_split_bound:407); Primitives.lean (Laws); Scheme.lean (falconPSF)', focus: 'soundness OF STATEMENTS: proven / sorry / vacuous / mis-stated. NOTE verify_sign_correct + F8 toRq_rqToIntPolyCentered are PROVEN (s10, standard-axioms-only) — re-verify not re-flag; euf_cma_security still sorry + discards hyps (self-labeled CONJECTURAL); which GPV sorrys are load-bearing; is Correct falconPSF provable given SD-5; SamplerQuality non-vacuous; TS-4 two-world gap (byte-level Falcon.verify vs GPV verify) still open.' },
