...
appearing as natural numbers in a numerical loss.
-/
...
/-- The scheme's generated-key distribution, packaged with definitional public seed/root
coherence.  Security experiments use this distribution rather than an arbitrary sampler of
merely matching key records. -/
def generateKeyPair {p : Params} (prims : Primitives p)
    [SampleableType prims.SkSeed] [SampleableType prims.SkPrf]
    [SampleableType prims.PkSeed] : ProbComp (GeneratedKeyPair prims) := do
  let secretSeed ← $ᵗ prims.SkSeed
  let secretPrf ← $ᵗ prims.SkPrf
  let publicSeed ← $ᵗ prims.PkSeed
  let pair := slhKeygenInternal prims secretSeed secretPrf publicSeed
  return {
    publicKey := pair.1
    secretKey := pair.2
    seed_eq := rfl
...
inductive Query {p : Params} (prims : Primitives p) (_pk : PublicKey prims) where
...
@[reducible] def oracleSpec {p : Params} (prims : Primitives p) (pk : PublicKey prims) :
    OracleSpec (Query prims pk) :=
  OracleSpec.ofFn fun
    | .sign _ => Signature prims
    | .prf _ _ => prims.Y
...
inductive AdversaryQuery {p : Params} (prims : Primitives p) (_pk : PublicKey prims) where
  | sign (request : MessageInput)
...
@[reducible] def adversaryOracleSpec {p : Params} (prims : Primitives p)
    (pk : PublicKey prims) : OracleSpec (AdversaryQuery prims pk) :=
  OracleSpec.ofFn fun
    | .sign _ => Signature prims
    | .f _ _ => prims.Y
    | .h _ _ _ => prims.Y
...
def adversaryQueryImpl {p : Params} (prims : Primitives p) (pk : PublicKey prims) :
    QueryImpl (adversaryOracleSpec prims pk) (OracleComp (oracleSpec prims pk))
  | .sign request => liftM ((oracleSpec prims pk).query (.sign request))
  | .f address input => liftM ((oracleSpec prims pk).query (.f .adversary address input))
  | .h address left right =>
      liftM ((oracleSpec prims pk).query (.h .adversary address left right))
  | .tlFors address inputs =>
      liftM ((oracleSpec prims pk).query (.tlFors .adversary address inputs))
  | .tlWots address inputs =>
      liftM ((oracleSpec prims pk).query (.tlWots .adversary address inputs))
  | .hmsg randomizer request =>
      liftM ((oracleSpec prims pk).query (.hmsg .adversary randomizer request))

/-- Exactly the public signing-query variant. -/
def isSigningQuery {p : Params} {prims : Primitives p} {pk : PublicKey prims} :
    AdversaryQuery prims pk → Prop
  | .sign _ => True
  | _ => False

instance {p : Params} {prims : Primitives p} {pk : PublicKey prims} :
    DecidablePred (isSigningQuery (prims := prims) (pk := pk)) := fun query => by
...
def isHashQuery {p : Params} {prims : Primitives p} {pk : PublicKey prims} :
    AdversaryQuery prims pk → Prop
  | .f _ _ | .h _ _ _ | .tlFors _ _ | .tlWots _ _ | .hmsg _ _ => True
  | _ => False

instance {p : Params} {prims : Primitives p} {pk : PublicKey prims} :
    DecidablePred (isHashQuery (prims := prims) (pk := pk)) := fun query => by
...
def queryImpl {p : Params} {prims : Primitives p} (keys : GeneratedKeyPair prims)
    (encode : MessageInput → List Byte)
    (sign : SecretKey prims → MessageInput → ProbComp (Signature prims)) :
    QueryImpl (oracleSpec prims keys.publicKey) ProbComp
  | .sign request => sign keys.secretKey request
...
structure ClassicalAdversary {p : Params} (prims : Primitives p) where
  main (pk : PublicKey prims) :
    OracleComp (adversaryOracleSpec prims pk) (MessageInput × Signature prims)

/-- Pathwise signing-query bound on an actual adversary computation. -/
def SigningBound {p : Params} {prims : Primitives p} {pk : PublicKey prims} {α : Type}
    (program : OracleComp (adversaryOracleSpec prims pk) α) (qS : ℕ) : Prop :=
  program.IsQueryBoundP isSigningQuery qS

/-- Pathwise public-hash-query bound on an actual adversary computation. -/
def HashQueryBound {p : Params} {prims : Primitives p} {pk : PublicKey prims} {α : Type}
    (program : OracleComp (adversaryOracleSpec prims pk) α) (qH : ℕ) : Prop :=
...
def AdversaryBounds {p : Params} {prims : Primitives p}
    (adversary : ClassicalAdversary prims) (qS qH : ℕ) : Prop :=
...
@[simp]
theorem signingBound_query_iff {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    (request : MessageInput) (qS : ℕ) :
    SigningBound
      (liftM ((adversaryOracleSpec prims pk).query (.sign request)) :
        OracleComp (adversaryOracleSpec prims pk) (Signature prims)) qS ↔ 0 < qS := by
  simp [SigningBound, isSigningQuery, OracleComp.isQueryBoundP_query_iff]

@[simp]
theorem hashQueryBound_f_iff {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    (address : Adrs) (input : prims.Y) (qH : ℕ) :
    HashQueryBound
      (liftM ((adversaryOracleSpec prims pk).query (.f address input)) :
        OracleComp (adversaryOracleSpec prims pk) prims.Y) qH ↔ 0 < qH := by
  simp [HashQueryBound, isHashQuery, OracleComp.isQueryBoundP_query_iff]

theorem not_signingBound_zero {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    (request : MessageInput) :
    ¬ SigningBound
      (liftM ((adversaryOracleSpec prims pk).query (.sign request)) :
        OracleComp (adversaryOracleSpec prims pk) (Signature prims)) 0 := by
  simp

theorem not_hashQueryBound_f_zero {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    (address : Adrs) (input : prims.Y) :
    ¬ HashQueryBound
      (liftM ((adversaryOracleSpec prims pk).query (.f address input)) :
        OracleComp (adversaryOracleSpec prims pk) prims.Y) 0 := by
  simp

@[simp]
theorem signingBound_pure_zero {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    {α : Type} (value : α) :
    SigningBound (pure value : OracleComp (adversaryOracleSpec prims pk) α) 0 := by
  simp [SigningBound]

@[simp]
theorem hashQueryBound_pure_zero {p : Params} {prims : Primitives p} (pk : PublicKey prims)
    {α : Type} (value : α) :
    HashQueryBound (pure value : OracleComp (adversaryOracleSpec prims pk) α) 0 := by
  simp [HashQueryBound]
