...
component hash, so the three FORS games a downstream reduction has to reach are the same three;
which forgeries reach which is what changes.  Whether the source's summand coefficients survive that
