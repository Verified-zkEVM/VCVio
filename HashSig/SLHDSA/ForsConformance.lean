...
introduced. -/
@[simp] theorem forsAuthPathVector_decode_get {Y : Type} (p : Params)
    (leaf : ℕ → Y) (nodeHash : ℕ → ℕ → Y → Y → Y)
    (md : ForsDigest p) (tree : TreeIndex p) (j : Fin p.a) :
    (forsAuthPathVector leaf nodeHash
  ...
  change (forsAuthPathVector leaf nodeHash idx p.a)[j.val] =
    PerfectMerkleTree.merkleRoot leaf nodeHash j.val
      (PerfectMerkleTree.sibling (idx / 2 ^ j.val))
  have hlist := forsAuthPathVector_toList leaf nodeHash idx p.a
  have hget := congrArg (fun xs => xs.getD j.val (leaf 0)) hlist
  have hv : j.val < (forsAuthPathVector leaf nodeHash idx p.a).toList.length := by simp
