...
The one-byte type hypothesis is stated separately because `ADRSc` compresses the full four-byte
ADRS type field.  `Adrs.fits_one_type_of_isCanonical` discharges it from canonicality, which also
supplies the three four-byte word bounds. -/
